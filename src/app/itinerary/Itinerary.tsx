...
import { GenerateItineraryOutput, UserPreferences } from "../ai/generateItinerary";
import { ReactNode, useEffect, useState } from "react";
import destinations from "./data";
import { Input, Popover, PopoverContent, PopoverHandler } from "@material-tailwind/react";
import { reviseItinerary } from "./actions";
          ...
          <div className="my-4" key={i}>
            <div className="flex items-center">
              ...
              <Popover placement="top-end">
                ...
                <PopoverContent className="max-w-64 py-2 px-3 text-xs bg-gray-900 text-white">
                  <div>{a.rationale}</div>
                </PopoverContent>
              </Popover>
              {a.mustDo && (
                <Popover placement="top-end">
                  ...
                  <PopoverContent className="py-2 px-3 text-xs bg-gray-900 text-white">
                    <div>{a.mustDo}</div>
                  </PopoverContent>
                </Popover>
              )}
            ...
            {revisionNotes[i] && (
              <form
                ...
                onSubmit={async (e) => {
                  e.preventDefault();
                  const notes = new FormData(e.target).get("revisionNotes")?.toString();
                  if (!!notes?.trim()) {
                    isRevising[i] = true;
              ...
              >
                <Input
                  ...
                  onInput={(e) => {
                    revisionNotes[i] = e.target.value;
                    setRevisionNotes([...revisionNotes]);
                  }}
                />
              </form>
            ...
            {a.ref && (
              <div className="rounded-t-xl overflow-hidden">
                <ImageForRef destination={destination} activity={a.ref} />
              </div>
            )}
            <div
